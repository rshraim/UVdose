...
#' @return A numeric vector of mean or cumulative seasonal erythemal UV dose estimates.
#' @examples
...
season_uve <- function(data, date, longitude, latitude, temis_path=getwd(), season="summer", type="cumulative", region="europe"){
  ...
  #### cumulative or average seasonal erythemal UV ####
  ...
  season_uv <- function(coords_val, date_val) {
    ...
    uv_window <- merythema_clim_all[(n):(n - 364), c("uvd.x", "date")]
    uv_filtered <- dplyr::filter(uv_window, lubridate::month(date) %in% num_season)
    summary_fun(uv_filtered$uvd.x, na.rm = TRUE)
  }

  #apply over rows
  ery <- purrr::pmap_dbl(list(data_tmp$coords, data_tmp$date), season_uv)
