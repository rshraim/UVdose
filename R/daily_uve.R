...
#' @return A numeric vector of ambient daily erythemal UV measurements.
#' @examples
...
daily_uve <- function(data, date, longitude, latitude, temis_path=getwd(), region="europe"){
  ...
  #### Extract erythemal UV on given date ####

  merythema_clim_all <- dplyr::arrange(merythema_clim_all, .data$coords, .data$date)

  d_ery <- dplyr::left_join(data_tmp, merythema_clim_all[, c("coords", "date", "uvd.x")], by = c("coords", "date")) %>%
    dplyr::pull("uvd.x")
