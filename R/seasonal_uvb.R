...
#' @param season The duration over which erythemal UV dose is to be estimated. One of "summer" covering April to September (default), "winter" covering October to March, or a numeric vector corresponding to month range.
#' @param type Type of annual dose to be calculated, cumulative (default) or mean.
#' @return A numeric vector of mean or cumulative seasonal UVB dose estimates.
#' @examples
...
season_uvb <- function(data, date, longitude, latitude, temis_path=getwd(),  season="summer", type="cumulative", region="europe"){
  ...
  #### Extract UVB for given season or month range ####
  mvitd_clim_all <- dplyr::arrange(mvitd_clim_all, .data$coords, .data$date)

  #define selected season months
  num_season <- if (is.numeric(season)) {
    season
  ...
  summary_fun <- switch(type,
                        "cumulative" = sum,
                        "mean" = mean,
                        stop("Invalid type input, must be 'cumulative' or 'mean'.")
                        )

  #function to calculate seasonal dose
  season_uv <- function(coords_val, date_val) {
    n <- which(mvitd_clim_all$coords == coords_val & mvitd_clim_all$date == date_val)
    if (length(n) == 0 || n - 364 < 1) return(NA_real_)

    uv_window <- mvitd_clim_all[(n):(n - 364), c("uvd.x", "date")]
    uv_filtered <- dplyr::filter(uv_window, lubridate::month(date) %in% num_season)
    summary_fun(uv_filtered$uvd.x, na.rm = TRUE)
  }

  #apply over rows
  d_uvb <- purrr::pmap_dbl(list(data_tmp$coords, data_tmp$date), season_uv)
