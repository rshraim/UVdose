...
#' @param date date vector, usually date of assessment or recruitment.
...
#' @return A numeric vector of mean or cumulative annual vitamin D UVB dose estimates.
#' @examples
...
annual_uvb <- function(data, date, longitude, latitude, temis_path=getwd(), type="cumulative", region="europe"){
  ...
  uv_year <- function(coords_val, date_val) {
    ...
    uv_vals <- mvitd_clim_all[(n):(n - 364), "uvd.x", drop = TRUE]
    summary_fun(uv_vals, na.rm = TRUE)
  }

  #apply uv_dose over rows
  duvb <- purrr::pmap_dbl(list(data_tmp$coords, data_tmp$date), uv_year)
